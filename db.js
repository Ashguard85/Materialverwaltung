...
async function seedLocalDb(db) {
  ...
  if (!seeded) {
    ...
    for (const name of ['Mikrocontroller', 'Sensoren', 'Module', 'Widerstände', 'Kondensatoren', 'Mechanik', 'Sonstiges']) {
      tx.objectStore('categories').put({ id: crypto.randomUUID(), name, created_at: now, updated_at: now });
    }
    for (const name of ['Werkbank', 'Schublade 1', 'Schublade 2']) {
      tx.objectStore('locations').put({ id: crypto.randomUUID(), name, created_at: now, updated_at: now });
    }
...
export async function localGetAll(store) {
  ...
  const result = await requestAsPromise(tx.objectStore(store).getAll());
  await transactionDone(tx);
  db.close();
...
export async function localDelete(store, id) {
  const db = await openLocalDb();
  let stores = [store];
  if (store === 'items' || store === 'projects') stores = [store, 'project_items'];
  if (store === 'categories' || store === 'locations') stores = [store, 'items'];
  const tx = db.transaction(stores, 'readwrite');
  tx.objectStore(store).delete(id);
  if (store === 'items' || store === 'projects') {
    const piStore = tx.objectStore('project_items');
    const all = await requestAsPromise(piStore.getAll());
    for (const row of all) {
      if ((store === 'items' && row.item_id === id) || (store === 'projects' && row.project_id === id)) piStore.delete(row.id);
    }
  }
  if (store === 'categories' || store === 'locations') {
    const itemStore = tx.objectStore('items');
    const allItems = await requestAsPromise(itemStore.getAll());
...
export async function getSecureSetting(key) {
  ...
  const row = await requestAsPromise(tx.objectStore('settings').get(key));
  await transactionDone(tx);
  db.close();
...
export async function clearServerCredentials() {
  for (const key of ['backendUrl', 'dockerWebUrl', 'cfClientId', 'cfClientSecret', 'appApiToken']) await deleteSecureSetting(key);
}
