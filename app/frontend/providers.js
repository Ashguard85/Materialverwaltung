...
export class LocalProvider {
  ...
  async exportData() { const data = stripImageMeta(await this.bootstrap()); data.projects = data.projects.map(row => ({ ...row, image_mime_type: '', image_updated_at: '' })); return { format: BACKUP_FORMAT, version: BACKUP_VERSION, exported_at: now(), app_version: 'v13', includes_images: false, includes_project_files: false, data }; }
